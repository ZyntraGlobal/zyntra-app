...
//
// Os dados reais moram num repositório PRIVADO separado (zyntra-app-data) —
// este script lê/escreve neles via API do GitHub (GH_DATA_TOKEN, secret do repo).
...
const HORAS_ALVO = [8];
const STATE_FILE = 'notif-state-diaria.json';
...
async function main() {
  ...
  if (okCount > 0) {
    ...
    await ghPutJSON(STATE_FILE, { dia: hoje, enviados: passados }, stateSha, 'Atualiza estado da notificacao diaria').catch(e => console.log('Aviso: falha ao salvar estado:', e.message));
  } else {
    process.exitCode = 1;
  }
}
